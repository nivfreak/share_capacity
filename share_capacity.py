...
def process_exposure_item(item, protocol, exposure_type, name_key, fs_path_key, free_space, usable_space, conn):
    ...
    Returns a list:
    [protocol, exposure_type, name, fs_path, used_space, free_space, usable_space, used_pct]
    """
    ...
    return [protocol, exposure_type, name, fs_path, used_space, free_space, usable_space, used_pct]
...
def deduplicate_rows(data_rows):
    ...
    fs_path, used_space, free_space, usable_space, used_pct, exposures (a list of exposure strings).
    """
    dedup = {}
    for row in data_rows:
        # row = [protocol, exposure_type, name, fs_path, used_space, free_space, usable_space, used_pct]
        key = row[3]  # filesystem path
        exposure = f"{row[0]}:{row[1]}:{row[2]}"
        if key in dedup:
            dedup[key]["exposures"].append(exposure)
        else:
            dedup[key] = {
                "fs_path": key,
                "used_space": row[4],
...
def main():
    ...
    # Compute the date once and embed it in the filename.
    ...
    # Process SMB shares.
    for share in get_smb_shares(conn):
    ...
    # Process NFS exports.
    for export in get_nfs_exports(conn):
        row = process_exposure_item(
            export,
            protocol="NFS",
    ...
    # Process S3 buckets.
    for bucket in get_s3_buckets(conn):
        row = process_exposure_item(
            bucket,
            protocol="S3",
    ...
    # Print summary of processed entries.
    count_smb = len(get_smb_shares(conn))
    count_nfs = len(get_nfs_exports(conn))
    count_s3 = len(get_s3_buckets(conn))
